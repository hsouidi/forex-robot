...
@Slf4j
@Service
public class MovingAverageCrossStrategyInLive1506 implements Strategy {
    ...
    public Trade check(Symbol symbol, Date to) {
        ...
        AlgoUtils.check(candleM5.getLast(), to, this.getClass().getSimpleName() + "  M5", 5);
        AlgoUtils.check(candleH1.getLast(), to, this.getClass().getSimpleName() + " H1", 60);
        AlgoUtils.check(candleM15.getLast(), to, this.getClass().getSimpleName() + " M15", 15);
    ...
    private Trade check(Symbol symbol, CustomList<Candle> candleM5, CustomList<Candle> candleH1, CustomList<Candle> candleD1, CustomList<Candle> candleM15) {
        ...
        final CustomList<Integer> crossM5Buy = getCrossList(candleM5, 20, 50, BUY, symbol);
    ...
    public boolean checkBooking(Trade trade, CustomList<Candle> candles) {
        ...
        final boolean  risqueReternement=barreDistance(currentPrice,1000,way,symbol)<20D;
        if (risqueReternement) {
            log.info("risque de retournement ");
            return false;
        }
        if ((way.getValue() * candles.getLast().body() > 2D)
                && ((way == BUY && last.getBody() > 0) || (way == Way.SELL && last.getBody() < 0))) {
