...
@Slf4j
//@Service
public class MovingAverageCrossStrategyInTest implements Strategy {
    ...
    private Trade check(Symbol symbol, CustomList<Candle> candleM5, CustomList<Candle> candleH1, CustomList<Candle> candleD1, CustomList<Candle> candleM15) {
        ...
        final double ema10M5 = valueDema(candleM5, 10);
        final double ema100M15 = valueEma(candleM15, 100);


        final boolean isEmaSell = //close<ema10M5 &&
                h1Close < ema100H1 && toPip(symbol, ema100H1 - close) > 10D && close < ema100M15 && M15Close < ema100M15;
        final boolean isEmaBuy =  //close>ema10M5 &&
                h1Close > ema100H1 && toPip(symbol, close - ema100H1) > 10D && close > ema100M15 && M15Close > ema100M15;
    ...
    public boolean checkBooking(Trade trade, CustomList<Candle> candles) {
        ...
        if ((way.getValue() * candles.getLast().body() > 2D)
            ...
            .stream().filter(candle -> candle.date().before(to)).collect(Collectors.toCollection(CustomList::new));
            CustomList<ZigZag.TrendInfo> zig = ZigZag.values(candleH1);
            ZigZag.TrendInfo trendInfo = zig.getLastMinus(1);
            FibonacciResult fibonacciResult = Fibonacci.values(zig);
            CustomList<FibonacciResult.Placement> placements=fibonacciResult.getPlacementHistory(candles.stream().filter(c->c.date().after(trendInfo.getEnd())).collect(Collectors.toList()));

            final double risque = way==Way.SELL?(placements.getLast().getAbove().getValue()-currentPrice):(currentPrice-placements.getLast().getBellow().getValue());
