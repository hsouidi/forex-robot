...
@Slf4j
//@Service
public class MovingAverageCrossStrategyInTESTNew implements Strategy {
    ...
    private Trade check(Symbol symbol, CustomList<Candle> candleM5, CustomList<Candle> candleH1, CustomList<Candle> candleD1, CustomList<Candle> candleM15) {
        ...
        CustomList<Double> para = toList(SAR.values(candleM5));
        ...
        if (
                (heikenAshiBarsM5.getLast().getBody() < 0
                        //|| heikenAshiBarsM15.subList(heikenAshiBarsM15.size() - 3, heikenAshiBarsM15.size() - 1).stream().anyMatch(c -> Way.BUY.getValue() * c.getBody() > 0)
                ) &&
                        isEmaBuy
            ...
            return new Trade(Way.BUY, symbol);
                ...
                (heikenAshiBarsM5.getLast().getBody() > 0
                        //|| heikenAshiBarsM15.subList(heikenAshiBarsM15.size() - 3, heikenAshiBarsM15.size() - 1).stream().anyMatch(c -> Way.SELL.getValue() * c.getBody() > 0)
                ) &&
                        isEmaSell
                        && crossM5Buy.getLast() > crossM5Sell.getLast()
        ) {  // SELL
    ...
    public boolean checkBooking(Trade trade, CustomList<Candle> candles) {
        ...
        Double std20=StandardDeviation.value(candles,20,1);
        Date to = candles.getLast().date();
        final CustomList<Candle> candleM15 = instrumentService.getPricing(CandlestickGranularity.M15, symbol,to, getFromDate(to, CandlestickGranularity.M15))
                .stream().filter(candle -> candle.getComplete()).collect(Collectors.toCollection(CustomList::new));
        CustomList<HeikenAshi.HeikenAshiBar> heikenAshiBarsM15 = HeikenAshi.OnCalculate(candleM15);
        HeikenAshi.HeikenAshiBar last15 = heikenAshiBarsM15.getLast();


        if ((way.getValue() * candles.getLast().body() > 2D)
                && ((way == Way.BUY && last.getBody() > 0 && last15.getBody() >0)
                || (way == Way.SELL && last.getBody() < 0 && last15.getBody() <0))) {
    ...
    public boolean checkStatus(Trade trade, CustomList<Candle> candles) {
        ...
        if (wayFactor * (closePrice - trade.getSupport()) < 0) {
            ...
            final CustomList<Candle> candleM5 = instrumentService.getPricing(CandlestickGranularity.M5, symbol, to, getFromDate(to, CandlestickGranularity.M5))
                    .stream().filter(candle -> candle.getComplete()).collect(Collectors.toCollection(CustomList::new));
            CustomList<HeikenAshi.HeikenAshiBar> heikenAshiBarsM5 = HeikenAshi.OnCalculate(candleM5);
            BolingerBands.BBOND bolingerBands = BolingerBands.value(candleM5, 20);
            ...
            return true;
/*        } else if (wayFactor * (closePrice - trade.getSupport()) > 0) {
            Date to = candles.getLast().date();
            final CustomList<Candle> candleM5 = instrumentService.getPricing(CandlestickGranularity.M5, symbol, to, getFromDate(to, CandlestickGranularity.M5))
                    .stream().filter(candle -> candle.getComplete()).collect(Collectors.toCollection(CustomList::new));
            CustomList<Double> para = toList(SAR.values(candleM5));
            if(wayFactor*(para.getLast()-para.getLastMinus(1))>0){
                return true;
            }

 */       } else if (wayFactor * (closePrice - trade.getResistance()) > 0) {
            final Double risque = trade.getRisque();
            trade.setSupport(trade.getResistance());
