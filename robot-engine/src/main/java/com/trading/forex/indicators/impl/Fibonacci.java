...
public class Fibonacci extends IndicatorUtils {
    ...
    public static FibonacciResult values(CustomList<ZigZag.TrendInfo> zigzag) {

        ZigZag.TrendInfo trendInfo = zigzag.getLastMinus(1);
        final Double low = trendInfo.getTrend() == Trend.UP ? trendInfo.getStartPrice() : trendInfo.getEndPrice();
        final Double high = trendInfo.getTrend() == Trend.DOWN ? trendInfo.getStartPrice() : trendInfo.getEndPrice();
        Function<Double, Double> retracement = null;
        Function<Double, Double> extention = null;
        final Way way = Trend.UP == trendInfo.getTrend() ? Way.BUY : Way.SELL;
        if (way == Way.BUY) {
        ...
        return FibonacciResult.builder()
                ...
                .ret_161_8(retracement.apply(1.618D))
                .ret_261_8(retracement.apply(2.618D))
                .ret_423_6(retracement.apply(4.236D))
                ...
                .ext_423_6(extention.apply(4.236D))
                .way(way)
                .build();
    }
    ...
    public static FibonacciResult values(CustomList<Candle> candles, int periode) {
        ...
        Function<Double, Double> retracement = null;
        Function<Double, Double> extention = null;
        final Way way = lowSearchResult.getIndex() == highSearchResult.getIndex() ? candles.getFirst().getClose() > candles.getFirst().getOpen() ? Way.BUY : Way.SELL : lowSearchResult.getIndex() < highSearchResult.getIndex() ? Way.BUY : Way.SELL;
        if (way == Way.BUY) {
            retracement = percentage -> high - ((high - low) * percentage);
            extention = percentage -> high + ((high - low) * percentage);
        ...
        return FibonacciResult.builder()
                ...
                .ext_200(extention.apply(2D))
                .ext_261_8(extention.apply(2.618))
                .ext_23_6(extention.apply(0.236D))
                .ext_38_2(extention.apply(0.382D))
